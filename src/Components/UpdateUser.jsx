...
import { AllCountry, SingleUserData, UpdateUserData } from '../axios/apis'
...
const UpdateUser = () => {
    ...
    const getCountryData = async () => {
        setLoader(true)
        try {
            const res = await SingleUserData(id)
            setUserData({
                ...
                dateBirth: spliceDate(res.data.data?.dateBirth),
            })
            const resCountry = await AllCountry()
            setCountry(resCountry.data.data)
            setState(resCountry.data.data.filter((item) => item.name === res.data.data?.country)[0]?.state)
            setLoader(false)
        } catch (error) {
            console.log(error)
        }
    ...
    setTimeout(() => {
        setCity(state?.filter((item) => item.name === userData?.state)[0]?.city)
        // setUserData({ ...userData, dateBirth: spliceDate(userData?.dateBirth) })
    }, 1000);
    // useEffect(() => {
    // }, [userData])
